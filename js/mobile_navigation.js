...
Weebly.mobile_navigation = (function($) {
	...
	var resizeViewport = function() {
		...
		if (navStyle == SLIDE_RIGHT && isOpen) {
			$menuWrapper.height('auto');
			var viewportHeight = Math.max($windowProxy.height() + browserPadding, $menuWrapper.outerHeight());
			...
			// android reflow hack
			$('#wsite-viewport').css({ 'overflow' : 'visible' });
			setTimeout(function(){
				$('#wsite-viewport').css({ 'overflow' : 'hidden' });
			},150);

			if ($menuWrapper.height() < viewportHeight) {
				$menuWrapper.css('height', viewportHeight + 'px');
			}

			$('#wsite-page-wrapper').css('height', viewportHeight + 'px');
		}

		if (!Weebly.mobile) {
			var height = $windowProxy.height() + browserPadding;
			$('.wsite-background').css({'height' : height + 'px'});
		}
	};
	...
	var resizeContentWindow = function() {
		var top = (isiOS) ? $(window).height() + IOS_ADDRESS_BAR_HEIGHT : $(window).height(),
			$footer = $('.wsite-mobile-footer').up();
	...
	var initMenu = function() {
		...
		$menuButton = $('.wsite-menu-button');
		$headerUl = $('.wsite-menu-default');

		$headerUl.wrap('<div class="wsite-mobile-menu" />');
		$menu = $headerUl.up('.wsite-mobile-menu');
		$menuWrapper = $headerUl.up('.wsite-menu-wrapper');
	...
	/**
	 * Resize iframe heights from event data
	 *
	 * @return undefined
	 */
	var resizeIframes = function() {
		var $mapFrames = $('.wmap-container');
		$mapFrames.find('iframe').css({ height : pendingResizeData + 'px'});
	...
	$(function() {
		// Check if in editor
		if (Weebly.mobile) {
			resizeContentWindow = Weebly.mobile.resizeScreen;
			return;
		}
		...
		if (pendingResizeData !== null) {
			resizeIframes();
		}

		$(window).on("resize", resizeContentWindow);

		resizeContentWindow();
		//Doesn't work on ios7
		window.scroll(0,1);
		$(window).load(resizeContentWindow);
	});
